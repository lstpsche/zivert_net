...
class MeasurementsDataLoader extends React.Component {
  fetchMeasurements () {
    ...
    fetchLink({
      ...
      onSuccess: (response) => {
        setMeasurements(
          response.measurements.map(({ data: { attributes: { id, longitude, latitude, value_urh, value_ush, user_id: userId, created_at: createdAt } } }) => (
            { id, longitude, latitude, value_urh, value_ush, userId, createdAt }
          ))
        );
      },
      onFailure: (error) => {
