...
describe StaticMeasurements::UpdationJob, type: :job do
  describe '#perform' do
    subject { described_class.perform_now(static_measurement) }

    let(:static_measurement) { instance_double(Measurement, json: static_measurement_json) }
    ...
    before do
      ...
      allow(action_cable_server).to receive(:broadcast)
        .with('static_measurements_channel', action: 'update', static_measurement: static_measurement_json)
    end
